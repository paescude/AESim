...
//					int yClerk= grid.getLocation(this).getY();					
	...
	public void initNumClerks() {
		...
		switch (id) {
		case 1:
			this.setMyShiftMatrix(Reader.getMatrixClerk1());
			for (int i = 0; i < 7; i++) {
				sum = 0;
				for (int j = 0; j < 23; j++) {
					sum = sum + Reader.getMatrixClerk1()[j][i];
				}
				this.durationOfShift[i] = sum;
			}
			break;
		case 2:
			this.setMyShiftMatrix(Reader.getMatrixClerk2());
			for (int i = 0; i < 7; i++) {
				sum = 0;
				for (int j = 0; j < 23; j++) {
					sum = sum + Reader.getMatrixClerk2()[j][i];
				}
				this.durationOfShift[i] = sum;
			}
			break;
